<x-app-layout>
    <div class="container mx-auto my-4 text-white">
        <h1 class="text-3xl font-semibold text-white mb-6">Edit Blog Post</h1>
        ...
        <div>
            <form action="{{ route('post.update', $post) }}" method="post">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="body" id="body" class="mt-1 p-2 w-full border rounded-md text-black" required rows="8">{{ $post->markdown }}</textarea>
                </div>
            ...
            @foreach ($images as $image)
                <div class="basis-1/4 box-content flex relative p-2">
                    ...
                    <label for="image{{ $loop->iteration }}">
                        <img src="{{ asset('storage/' . $image->image_path) }}" class="h-25">
                    </label>
                </div>
            @endforeach
        </div>
